...
| Evolution this iteration | Initial register created. R001 and R002 are the two risks the declared scope carries. Four further risks were identified by the ProjectManager and are now **adopted as first-class risks with identifiers R003–R006**, on the stakeholder's decision of 2026-09-17 (see *Risk Register*). The open question that recorded their identifier-less state is retired. No risk carries strategy `accept`. |
...
**On identifiers.** The `RNNN` family is **declared input** — assigned in the Work Order and copied exactly, never minted by a role. R001 and R002 are the two risks the declared scope carries. R003–R006 were identified by the ProjectManager and were initially recorded without identifiers, because composing one would produce a citation that looks verified and is not. On being asked whether those four should be adopted as first-class risks with identifiers, the stakeholder answered **"Adopt all four as first-class risks and assign them identifiers"** (2026-09-17). The identifiers R003, R004, R005 and R006 are assigned on that authority, sequentially after R001 and R002. All six risks are now first-class and traceable.
...
:R003 client-timestamp trust boundary\nModerate, exposure 4;
note right
...
:R004 featured invariant under concurrent HR edits\nModerate, exposure 4;
note right
  Confronted in ELABORATION, in the Design Model
  of UC-002. CON-018 declares it a system invariant,
  so it is enforced where the change is made.
end note
:R006 mandatory design vs closed declared scope\nModerate, exposure 4;
note right
...
| R003 | FR-012, CON-017, AC-005 | Derives | Design Model |
| R004 | FR-007, CON-018 | Derives | Design Model |
| R005 | CON-011; Development Case measurement policy | Derives | Iteration Plan |
| R006 | CON-013; declared scope | Derives | Design Model |
| Risk List | R001, R002, R003, R004, R005, R006 | Derives | Iteration Plan |
| Risk List | R001, R002, R003, R004, R005, R006 | Derives | Iteration Assessment |

**All six risks are now first-class and traceable.** R003–R006 carry identifiers on the stakeholder's authority of 2026-09-17 ("Adopt all four as first-class risks and assign them identifiers"), so each now carries a trace edge into the artifact that must act on it. R005's edge points at the Iteration Plan because the gate ceiling it describes is a planning constraint; R003, R004 and R006 point at the Design Model because each is discharged by a design decision in the use-case realization named in the mitigation table.

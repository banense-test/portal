...
| Evolution this iteration | Initial register created. R001 and R002 are the two risks the declared scope carries and are cited by their **declared** identifiers. Four further risks were identified by the ProjectManager from the declared constraints and the Development Case; they are recorded **without** R-family identifiers, because that family is declared input and is not minted by this role. **One open question is escalated to the stakeholder this round: who assigns identifiers to those four risks.** No risk carries strategy `accept`. |
...
**OPEN — who assigns identifiers to these four risks.** `[SCOPE_QUESTION — not declared, but potentially critical: the register's coverage of risks beyond the two the declared scope names. The four risks above are classified with full probability/impact/exposure/mitigation rigour, but they carry no identifier, because the RNNN family is declared input and is not minted by this role. Without an identifier they carry no trace edge, so a later reviewer cannot follow them from this register into the Design Model or into the Elaboration review. Assigning identifiers is the stakeholder's act, not this role's.]` This question is escalated to the stakeholder in this round. Until it is answered, the four risks remain classified and their mitigation actions remain binding on the Design Model of UC-001 and UC-002 and on the Elaboration review — but they are not first-class traceable risks.

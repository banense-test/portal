...
| Evolution this iteration | Initial register created. R001 and R002 carried from the declared scope; R003–R006 identified from the declared constraints and the Development Case. No risk carries strategy `accept`. |
...
**Why R003–R006 are in the register and not in a backlog.** Each one is a *variable that can take a value that endangers success* and each one changes a decision: R003 changes how the clocking endpoint validates what it accepts; R004 changes where the featured invariant is enforced; R005 changes whether a gate must be bounded; R006 changes whether a Change Request is raised before implementation. A risk that changes no decision is not recorded here.

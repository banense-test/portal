...
| 4 — Re-anchor the Risk List's magnitude bands on a confirmed basis | **Met** | R001's probability and impact are recorded as the analyst's unconfirmed estimates in the Risk Register and the Risk Classification section, and no band is anchored on them: the Significant and Moderate bands rest on R002's and R003's declared exposures, and the High band's lower boundary on R004's observed exposure of 12. The stakeholder's third refusal settles the question |
...
## Rework Required
This section accumulates one entry per iteration, recording the findings open at that close, the findings closed during it, and the adjustments the assessment forces on the next plan.

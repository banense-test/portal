...
| 8 | Software Architecture Document | F2 | **Major** | Deployment View | Keycloak is placed in a `cloud` node **outside** the "Corporate Network — intranet only (CON-008)" boundary. No declared input states where Keycloak is hosted, and STK-003 — which operates the internal Windows Server estate — is declared to operate Keycloak. An unmarked assumption on an integration boundary, awkward against CON-007 (no cloud). | State the location as fact if it is one, or mark `[ASSUMPTION — requires validation]` and escalate, or place Keycloak inside the corporate network consistent with STK-003. |
...
| Cross-artifact consistency | **Fail** | One substantive contradiction (UCM F1) and one unmarked boundary assumption (SAD F2) |
| SCM state | **Pass** | No open PR, no open issue, green build on `main` |

**Why Approved with Changes and not Rejected.** No Critical finding exists. Every artifact is substantively sound: the scope is faithfully bounded, the risk register is complete and correctly sequenced, the architecture's decomposition is justified by area of change with the reasoning recorded, and the requirements baseline places every declared input. The defects are of two kinds — a trace graph that under-reports what the artifacts declare, and one genuine cross-artifact contradiction — and both are correctable without reworking the substance of any artifact. Rejecting would discard a sound baseline over bookkeeping and one actor-list correction.

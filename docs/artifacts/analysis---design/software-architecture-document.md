...
|---|---|---|
| R001 — a change on Infrastructure's side breaks the portal | The portal depends on AD and Keycloak through two configuration-held boundaries | Accepted in advance (CON-021). The dependency is confined to COMP-006 and COMP-010, so a change on Infrastructure's side cannot break development, and the real values are substituted at deployment. R001's probability and impact are recorded in the Risk List as the analyst's estimates, which the stakeholder has declined to confirm; the architectural exposure does not depend on their value, because the dependency is confined to two components either way |
...
| R004 — the stand-in environment is not ready | Every use case depends on the two CON-028 seams | **Materialized; treatment failed twice.** The stand-in environment was not delivered at Iter-1 close and not at Iter-2 close, so no use case has been built or tested and no iteration has produced a verifiable increment. Re-assessed at Iter-2 close: probability raised on the observed 2-of-2 materialization rate, magnitude Significant → High, owner changed to the Integrator alone, and the treatment replaced by a **hard gate** — no work item that exercises a use case against the stand-in starts until it is delivered and recorded. The stand-in directory must also carry the worker-category link, because UC-008 filters by it (CON-013) and the category is the one field the portal owns (CON-016). **No architectural change follows from the materialization**: the seams are unchanged and the components behind them are unchanged; what is missing is the stand-in behind the seams, which is construction work, not design work |
...
**No prototype is planned for Elaboration either.** The two mechanisms above are validated by the stand-in environment, which is construction work owned by the Implementer and Integrator, not by a proof-of-concept. If a later iteration finds a technical unknown that the stand-in cannot exercise, the disposition is revised then, with the risk it retires named.

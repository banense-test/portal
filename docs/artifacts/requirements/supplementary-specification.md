...
| NFR-003 Availability Window | CON-007 | Derives | COMP-002 |
...
**Constraints carrying no design element.** CON-003, CON-006, CON-007, CON-008, CON-019, CON-020, CON-021, CON-027, CON-029, CON-030 and CON-032 impose no element of the design of their own: they are satisfied by the declared environment, by the operating arrangement or by project process. CON-003 is a readiness statement — the OIDC client is already registered in Keycloak and its credentials are with the development team, so login can be tested from day one — and it is referenced by IFC-001; it constrains no element. None of the eleven carries a row and none carries an edge. Every declared constraint CON-001..CON-032 is therefore accounted for: 21 carry a row above, 11 are named here.

**Label scope.** `NFR-001..NFR-005`, `FR-001..FR-009`, `AC-001..AC-006`, `CON-001..CON-032`, `STK-001..STK-004`, `BG-001..BG-003` and `R001..R003` are declared identifiers, copied exactly. `USA-`, `REL-`, `PERF-`, `SUP-`, `SEC-`, `IFC-` and `STD-` are document-local labels of this Supplementary Specification; they are not trace-graph elements and no trace is registered on them. The same rule governs the Traces To column: every entry there is a trace-graph element identifier (`COMP-NNN`) — no document section, no artifact name and no document-local label appears in it. The Designer owns `INT-NNN` and `CLS-NNN`, the Software Architect owns `COMP-NNN`, the Test Designer owns `TC-NNN` — none is minted here.

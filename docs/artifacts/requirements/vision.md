...
**FR-008's search dimensions.** FR-008 declares the search by name, department or office; CON-013 declares that the directory also filters by worker category. The two are not in conflict — CON-013 adds a filter dimension, it does not remove one — so the filter is realized in UC-008 (main flow step 5, alternative flow A5) rather than escalated. The category remains descriptive: it filters the directory and appears as a column of the CSV export, and nowhere else (CON-013).
